...
class SupervisedDataset(Dataset):
    ...
    def get_item_id(self, idx):
        ...
        if isinstance(item, dict):
            ...
            # 再次使用图片/视频名称作为ID
                ...
                elif isinstance(image_file, list) and image_file:
                    return os.path.splitext(os.path.basename(image_file[0]))[0]
            elif 'video' in item:
                video_file = item['video']
                if isinstance(video_file, str):
                    return os.path.splitext(os.path.basename(video_file))[0]
                elif isinstance(video_file, list) and video_file:
                    return os.path.splitext(os.path.basename(video_file[0]))[0]
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        data_dict = dict(
            ...
            labels=labels,
            item_id=self.get_item_id(i),  # 添加item_id字段
        )
...
def make_supervised_eval_data_module(model_id, processor, data_args):
    ...
    return dict(
        ...
        eval_dataset=eval_dataset,  # 现在返回实际的验证数据集
